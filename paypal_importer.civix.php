...
function _paypal_importer_civix_civicrm_install()
{
    ...
    if ($upgrader = _paypal_importer_civix_upgrader()) {
        $upgrader->onInstall();
    }
}
 ...
 */
function _paypal_importer_civix_civicrm_postInstall()
{
    _paypal_importer_civix_civicrm_config();
    if ($upgrader = _paypal_importer_civix_upgrader()) {
 ...
 */
function _paypal_importer_civix_civicrm_uninstall()
{
    _paypal_importer_civix_civicrm_config();
    if ($upgrader = _paypal_importer_civix_upgrader()) {
 ...
 */
function _paypal_importer_civix_civicrm_enable()
{
    _paypal_importer_civix_civicrm_config();
    if ($upgrader = _paypal_importer_civix_upgrader()) {
 ...
 * @return mixed
 */
function _paypal_importer_civix_civicrm_disable()
{
    _paypal_importer_civix_civicrm_config();
    if ($upgrader = _paypal_importer_civix_upgrader()) {
...
/**
 * @return CRM_PaypalImporter_Upgrader
 */
 ...
 * @return array
 */
function _paypal_importer_civix_find_files($dir, $pattern)
{
    ...
    while (!empty($todos)) {
        ...
        foreach (_paypal_importer_civix_glob("$subdir/$pattern") as $match) {
            if (!is_dir($match)) {
                $result[] = $match;
            }
        }
 ...
 */
function _paypal_importer_civix_civicrm_managed(&$entities)
{
    $mgdFiles = _paypal_importer_civix_find_files(__DIR__, '*.mgd.php');
    sort($mgdFiles);
 ...
 * @return bool
 */
function _paypal_importer_civix_insert_navigation_menu(&$menu, $path, $item)
{
 ...
 */
function _paypal_importer_civix_navigationMenu(&$nodes)
{
    if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
        _paypal_importer_civix_fixNavigationMenu($nodes);
    }
}
 ...
 * missing them.
 */
function _paypal_importer_civix_fixNavigationMenu(&$nodes)
{
    $maxNavID = 1;
    ...
    _paypal_importer_civix_fixNavigationMenuItems($nodes, $maxNavID, null);
}

function _paypal_importer_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID)
{
    ...
    foreach ($origKeys as $origKey) {
        ...
        if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
            _paypal_importer_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
        }
    }
}
 ...
 */
function _paypal_importer_civix_civicrm_entityTypes(&$entityTypes)
{
